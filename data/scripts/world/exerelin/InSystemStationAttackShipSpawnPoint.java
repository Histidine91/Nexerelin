...
public class InSystemStationAttackShipSpawnPoint extends BaseSpawnPoint
{
	...
	private void setFleetAssignments(CampaignFleetAPI fleet)
	{
		...
		if(stationTarget != null && ExerelinUtils.isValidBoardingFleet(fleet, true))
		{
            ...
            if(((StarSystemAPI)stationTarget.getStationToken().getContainingLocation()).getName().equalsIgnoreCase(((StarSystemAPI)getAnchor().getContainingLocation()).getName()) || (ExerelinData.getInstance().getSectorManager() != null && FactionDirector.getFactionDirectorForFactionId(fleet.getFaction().getId()).getTargetResupplyEntityToken() == null))
                fleet.setPreferredResupplyLocation(getAnchor());
            else
	...
	private Script createTestTargetScript() {
		return new Script() {
			public void run() {
                ...
                {
                    lastTimeCheck = Global.getSector().getClock().getTimestamp();
                    if(stationTarget != null && ExerelinUtils.boardStationAttempt(theFleet, stationTarget.getStationToken(), false, true))
                        boarding = false;
                    else
